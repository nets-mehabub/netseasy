...
class Order {
    ...
    public function createNetsTransaction($oOrder) {
        ...
        $oID = $this->update_ordernr(\oxRegistry::getSession()
        ...
        try {
            ...
            $oCommonHelper = \oxNew(CommonHelper::class);
            $api_return = $oCommonHelper->getCurlResponse($apiUrl, 'POST', json_encode($data));
    ...
    public function processOrder($oUser) {
        ...
        $oCommonHelper = \oxNew(CommonHelper::class);
        $api_return = $oCommonHelper->getCurlResponse($oCommonHelper->getApiUrl() . $paymentId, "GET");
        $response = json_decode($api_return, true);
        ...
        $oCommonHelper->getCurlResponse($oCommonHelper->getUpdateRefUrl($paymentId), 'PUT', json_encode($refUpdate));

        if (\oxRegistry::getConfig()->getConfigParam('nets_autocapture')) {
            $chargeResponse = $oCommonHelper->getCurlResponse($oCommonHelper->getApiUrl() . $paymentId, 'GET');
